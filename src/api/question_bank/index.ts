...
export const reqBankList = (
  page: number,
  number: number,
  search: string | null,
) =>
  request.get<any, any>(
    API.BANK_LIST_URL + `?page=${page}&number=${number}&search${search}`,
  )
...
export const reqGetQuestionBesideBank = (
  page: number,
  number: number,
  id: number,
) =>
  request.get<any, questionListResponseData>(
    API.GET_BESIDE_URL + `?page=${page}&number=${number}&id=${id}`,
  )

export const reqExcludeQuestion = (bank_id: number, question_id: number) =>
  request.delete<any, deleteBankResponseData>(
    API.EXCLUDE_QUESTION_URL + `?bank_id=${bank_id}&question_id=${question_id}`,
  )
