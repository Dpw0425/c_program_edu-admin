<template>
  <div>
    <el-card class="box-card">
      <el-button type="primary" size="default" icon="Plus" @click="addTag">添加分类</el-button>
      ...
      <el-table style="margin: 10px 0" border :data="tagList">
        ...
        <el-table-column label="分类管理">
          <template #="{ row, $index }">
            <el-button type="warning" size="small" icon="Edit" @click="updateTag(row)"></el-button>
            <el-button type="danger" size="small" icon="Delete" @click="deleteTag(row)"></el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog v-model="deleteDialogForm" title="删除分类">
      <h2>即将删除分类<span style="color: red;">{{ del.name }}</span>, 是否确认？</h2>
...
import type { addTagForm, addTagResponse, deleteTagResponse, tagItem, TagList, tagListResponseData, updateTagForm, updateTagResponse } from '@/api/tag/type'
...
const rules = {
  name: [
    ...
    { min: 2, message: '分类名称不得少于两个字符！', trigger: ['blur', 'change'] },
    { max: 8, message: '分类名称不得多于八个字符！', trigger: ['blur', 'change'] },
  ],
}
